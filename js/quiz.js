...
const firebaseConfig = {
    apiKey: "AIzaSyBEGKn8KjEr6V_z92jLOFODHrIP-QNh6LI",
    authDomain: "quiz-prueba-db805.firebaseapp.com",
    projectId: "quiz-prueba-db805",
    storageBucket: "quiz-prueba-db805.firebasestorage.app",
    messagingSenderId: "455218781268",
    appId: "1:455218781268:web:7adf5304c016c0a734e1c1",
    measurementId: "G-CPBXYNDNBS"
};
...
document.getElementById("btn-name").addEventListener("click", () => {
    ...
    const originalText = btnName.innerText;
    
    btnName.innerText = "Verificando operador...";
    btnName.disabled = true;

    db.collection("ranking").where("alias", "==", inputName).get()
        .then((querySnapshot) => {
            btnName.innerText = originalText;
            btnName.disabled = false;

            if (!querySnapshot.empty) {
                alert(`Bloqueo de Seguridad: El alias "${inputName}" ya completó la simulación en el servidor. Por favor, elige otro alias.`);
            } else {
                userName = inputName;
        ...
        .catch((error) => {
            ...
            alert("Error de conexión con el servidor central. Intenta nuevamente.");
            btnName.innerText = originalText;
            btnName.disabled = false;
        });
});
...
function showResults() {
    ...
    document.getElementById("ranking-body").innerHTML = "<tr><td colspan='4'>Sincronizando con el servidor central...</td></tr>";

    db.collection("ranking").add({
        alias: userName,
        score: score,
        time: parseFloat(totalTimeSeconds),
        timestamp: firebase.firestore.FieldValue.serverTimestamp()
    }).then(() => {
        fetchGlobalRanking();
    }).catch((error) => {
        console.error("Error al guardar en Firebase:", error);
        document.getElementById("ranking-body").innerHTML = "<tr><td colspan='4'>Error de conexión. Contacte a soporte.</td></tr>";
    });
}
...
function fetchGlobalRanking() {
    db.collection("ranking")
        ...
        .then((querySnapshot) => {
            ...
            top10.forEach((player) => {
                ...
                if (player.alias === userName) {
                    tr.style.background = "rgba(0, 255, 156, 0.15)";
        ...
        .catch((error) => {
            ...
            document.getElementById("ranking-body").innerHTML = "<tr><td colspan='4'>No se pudo cargar el ranking.</td></tr>";
        });
}
